...
def get_users():
    ...
    admin_management_logger.info("Admin fetched all users.")
...
def update_admin_profile():
    ...
    if not admin:
        admin_management_logger.warning(f"Admin profile update failed: Admin {current_admin_id} not found.")
    ...
    if 'username' in data:
        ...
        if not validate_username(username):
            ...
            return jsonify({'message': 'Username can only contain lowercase letters, numbers, and underscores'}), 400
        if Admin.query.filter(Admin.id != current_admin_id, Admin.username == username).first():
    ...
    if 'email' in data:
        ...
        if Admin.query.filter(Admin.id != current_admin_id, Admin.email == email).first():
