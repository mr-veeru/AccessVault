...
class Config:
    ...
    JWT_ACCESS_TOKEN_EXPIRES = timedelta(minutes=int(os.getenv("JWT_ACCESS_TOKEN_EXPIRES", "15").replace("m", "")))  # JWT token expiration settings
    JWT_REFRESH_TOKEN_EXPIRES = timedelta(days=int(os.getenv("JWT_REFRESH_TOKEN_EXPIRES", "7").replace("d", "")))  # JWT refresh token expiration settings
